...
def get_gpus(nodes=None):
    ...
    lines = list()
    for node in nodes:
        output = subprocess.check_output(shlex.split("ssh %s "%(node)+NVIDIA_SMI_GET_GPUS))
        lines += output.decode("utf-8").split(os.linesep)
...
def get_gpu_processes(nodes=None):
    ...
    for node in nodes:
        output = subprocess.check_output(shlex.split("ssh %s "%(node)+NVIDIA_SMI_GET_PROCS))
        lines += output.decode("utf-8").split(os.linesep)

    gpu_uuid_to_id_map = {gpu.uuid: gpu.id for gpu in get_gpus(nodes)}
    processes = [
        _get_gpu_proc(line, gpu_uuid_to_id_map) for line in lines if line.strip()
    ]
    return processes
